...
//        }
    ...
    private void checkout(String address, String zip, String state, String city) {
        ...
        try {
            ...
            for (int i = 0; i < itemsList.size(); i++) {
                ...
                j.put("qty", String.valueOf(itemsList.get(i).getCount()));
                totalQuantity += itemsList.get(i).getCount()+1;
                totalPrice += itemsList.get(i).getCount() * Float.parseFloat(itemsList.get(i).getPrice());

                j.put("price", itemsList.get(i).getPrice());
                j.put("item", itemsList.get(i).getName());

                jsonItems.put(itemsList.get(i).getId(), j);
            }

            jsonItems.put("totalQty",String.valueOf(totalQuantity));
            jsonItems.put("totalPrice",String.valueOf(totalPrice));


            body.put("token", Constants.token);

            body.put("cart", jsonItems);
            body.put("address", address);
            body.put("totalQty", String.valueOf(totalQuantity));
            body.put("totalPrice", String.valueOf(totalPrice));
            body.put("state", state);
            body.put("city", city);
            body.put("zip_code", zip);

            Log.d(TAG, "checkout: " + body.toString());
    ...
    private void proceedOrder(JSONObject body) {
